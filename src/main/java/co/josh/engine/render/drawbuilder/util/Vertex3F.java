...
public class Vertex3F {
    ...
    Vertex3F(Vector3f pos, Vector4f col, Vector2f texcoords){
        this.position = pos;
        this.color = col;
    ...
    public Vertex3F pos(float x, float y, float z){
        return new Vertex3F(new Vector3f(x, y, z), color, texcoords);
    }

    public Vertex3F col(float r, float g, float b, float a){
        return new Vertex3F(position, new Vector4f(r, g, b, a), texcoords);
    }

    public Vertex3F uv(float x, float y){
        return new Vertex3F(position, color, new Vector2f(x, y));
    }
